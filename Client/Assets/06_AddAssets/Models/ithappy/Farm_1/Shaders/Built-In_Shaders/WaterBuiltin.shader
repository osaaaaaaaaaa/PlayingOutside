    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void FragmentFunction(Interpolators varyings, out half4 outColor : SV_Target)
            {
	            ...
	            half depth = Linear01Depth(tex2D(_CameraDepthTexture, uv_ss)) * _ProjectionParams.z - (varyings.position_ss.w - 1);
	            half depthMask = saturate(depth - _Depth);

	            // Calculating Refracted Depth
	            half refDepth = Linear01Depth(tex2D(_CameraDepthTexture, uv_ss + normal.xz * _Refraction)) * _ProjectionParams.z - (varyings.position_ss.w - 1);
	            ...
	            // Specular Coloring
	            half3 halfVector = normalize(_WorldSpaceLightPos0.xyz + viewDir);
